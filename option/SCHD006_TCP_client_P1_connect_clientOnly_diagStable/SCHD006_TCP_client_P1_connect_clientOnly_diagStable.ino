...
static inline void diagPeriodicLog() {
  ...
  DIAG_PRINTF("[DIAG] up=%lus freeHeap=%u minFree=%u tcp=%d\n",
              (unsigned long)(now/1000),
              (unsigned)ESP.getFreeHeap(),
              (unsigned)ESP.getMinFreeHeap(),
              (hostClient.connected() ? 1 : 0));
#endif
}
...
static inline void ensureTcpConnected() {
  ...
  const unsigned long now = millis();
  // connect() は失敗時にブロックし得るため、リトライ間隔を設ける
  if (now - lastAttempt < 5000) return;
  lastAttempt = now;

  hostClient.stop();
  DIAG_PRINTF("Attempting TCP connect to host %s:%d\n", hostIP.toString().c_str(), hostPort);
  if (hostClient.connect(hostIP, hostPort)) {
    DIAG_PRINTLN("Connected to host");
    hostClient.setTimeout(20); // read系ブロック抑制
  } else {
    DIAG_PRINTLN("Failed to connect to host");
  }
}
...
static inline int buildCsv18(char* out, size_t outSize) {
  // ホストの index.html で使っている18フィールドの並びに完全一致（データ内容は変更しない）
...
static inline void sendStateToHost() {
  ...
  // 1文字(近接) + 18フィールドCSV（String連結を避けてヒープ断片化を抑制）
  char csv[256];
  const int csvLen = buildCsv18(csv, sizeof(csv));
  if (csvLen <= 0 || csvLen >= (int)sizeof(csv)) return;

  char line[300];
  const char nearChar = (is_near_top ? '1' : '0');
  const int n = snprintf(line, sizeof(line), "%c,%s", nearChar, csv);
...
static inline void readSettingsFromHost() {
  ...
  while (hostClient.connected() && hostClient.available()) {
    hostClient.setTimeout(20);
    char msgBuf[64];
    size_t len = hostClient.readBytesUntil('\n', (uint8_t*)msgBuf, sizeof(msgBuf)-1);
    if (len == 0) continue;
    msgBuf[len] = '\0';
    // trim \r / spaces
    while (len > 0 && (msgBuf[len-1] == '\r' || msgBuf[len-1] == ' ' || msgBuf[len-1] == '\t')) { msgBuf[len-1] = '\0'; --len; }
